...
class Tournament:
    def __init__(
        self, name, place, date, time_control, nb_of_rounds=4, description=None
    ):
        self.name = name
        self.place = place
        self.date = date
    ...
    def table_round(self, id_tournament):
        ...
        for round in tournament["rounds"]:
            round_id += 1
            home.print_rounds_name(
                round["round_name"],
                round_id,
                round["date_time_start"],
                round["date_time_end"],
            )
    ...
    def table_match(self, round_name, choice_id_round):
        ...
        for match in round["matchs"]:
            cpt += 1
            home.print_match_players(
                cpt, match["player_one"]["name"], match["player_two"]["name"]
            )
